...
function createTempEnv(opts?: {
  ...
  const taskId = `prr-r-${id}`;
  const tmpBase = join(process.cwd(), 'tmp');
  if (!existsSync(tmpBase)) {
    mkdirSync(tmpBase);
  }
  const tmpDir = mkdtempSync(join(tmpBase, `prr-r-${id}-`));
...
describe('cli real-repo-pr-readiness', () => {
  test('missing taskId refuses', () => {
    const result = runCli(['real-repo-pr-readiness']);
  ...
  test('missing opt-in refuses', () => {
    ...
    try {
      ...
      assert.notStrictEqual(result.status, 0);
      assert(result.stderr.includes('ALLOW_REAL_REPO_PR_READINESS=true is required'), `Expected opt-in required: ${result.stderr}`);
    } finally {
      cleanup();
    }
  });
  ...
  test('missing task refuses', () => {
    ...
    try {
      const result = runCli(['real-repo-pr-readiness', 'nonexistent-task'], {
        TASKS_FILE: tasksFilePath,
  ...
  test('missing base_branch refuses', () => {
    const { taskId, tasksFilePath, cleanup } = createTempEnv();
    try {
      const content = readFileSync(tasksFilePath, 'utf-8');
  ...
  test('unknown commit_sha refuses', () => {
    const { taskId, tasksFilePath, runsDir, cleanup } = createTempEnv({ withState: true, withApprovalReport: true, stateOverrides: { commit_sha: 'aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa' } });
    try {
      const result = runCli(['real-repo-pr-readiness', taskId], {
        TASKS_FILE: tasksFilePath,
        ALLOW_REAL_REPO_PR_READINESS: 'true',
  ...
  test('missing approval-report.md refuses', () => {
    ...
    try {
      ...
      assert.notStrictEqual(result.status, 0);
      assert(result.stderr.includes('Approval report is required before PR readiness'), `Expected approval report required: ${result.stderr}`);
    } finally {
      cleanup();
    }
  });

  test('valid pushed state + approval report writes pr-readiness.md', () => {
    const { taskId, tasksFilePath, runsDir, cleanup } = createTempEnv({ withState: true, withApprovalReport: true });
    try {
      const result = runCli(['real-repo-pr-readiness', taskId], {
        TASKS_FILE: tasksFilePath,
        ALLOW_REAL_REPO_PR_READINESS: 'true',
      ...
      const reportPath = join(runsDir, taskId, 'pr-readiness.md');
      assert(existsSync(reportPath), `Expected pr-readiness.md to exist`);
    } finally {
      cleanup();
    }
  });

  test('valid pushed state + approval report writes pr-body.md', () => {
    const { taskId, tasksFilePath, runsDir, cleanup } = createTempEnv({ withState: true, withApprovalReport: true });
    try {
      runCli(['real-repo-pr-readiness', taskId], {
        TASKS_FILE: tasksFilePath,
        ALLOW_REAL_REPO_PR_READINESS: 'true',
        RUNS_DIR: runsDir,
      });
      const bodyPath = join(runsDir, taskId, 'pr-body.md');
      assert(existsSync(bodyPath), `Expected pr-body.md to exist`);
    } finally {
      cleanup();
    }
  });

  test('pr-readiness report contains task id', () => {
    const { taskId, tasksFilePath, runsDir, cleanup } = createTempEnv({ withState: true, withApprovalReport: true });
    try {
      runCli(['real-repo-pr-readiness', taskId], {
        TASKS_FILE: tasksFilePath,
      ...
      const report = readFileSync(join(runsDir, taskId, 'pr-readiness.md'), 'utf-8');
  ...
  test('pr-readiness report contains base branch', () => {
    const { taskId, tasksFilePath, runsDir, cleanup } = createTempEnv({ withState: true, withApprovalReport: true });
    try {
      runCli(['real-repo-pr-readiness', taskId], {
        TASKS_FILE: tasksFilePath,
  ...
  test('pr-readiness report contains work branch', () => {
    const { taskId, tasksFilePath, runsDir, cleanup } = createTempEnv({ withState: true, withApprovalReport: true });
    try {
      runCli(['real-repo-pr-readiness', taskId], {
        TASKS_FILE: tasksFilePath,
  ...
  test('pr-readiness report contains pushed commit SHA', () => {
    const { taskId, tasksFilePath, runsDir, cleanup } = createTempEnv({ withState: true, withApprovalReport: true });
    try {
      runCli(['real-repo-pr-readiness', taskId], {
        TASKS_FILE: tasksFilePath,
  ...
  test('pr-readiness report contains approval report path', () => {
    const { taskId, tasksFilePath, runsDir, cleanup } = createTempEnv({ withState: true, withApprovalReport: true });
    try {
      runCli(['real-repo-pr-readiness', taskId], {
        TASKS_FILE: tasksFilePath,
      ...
      const report = readFileSync(join(runsDir, taskId, 'pr-readiness.md'), 'utf-8');
      assert(report.includes('approval-report.md'), `Expected approval report path in report`);
    } finally {
      cleanup();
    }
  });

  test('pr-readiness report contains PR title suggestion', () => {
    const { taskId, tasksFilePath, runsDir, cleanup } = createTempEnv({ withState: true, withApprovalReport: true });
    try {
      runCli(['real-repo-pr-readiness', taskId], {
        TASKS_FILE: tasksFilePath,
      ...
      const report = readFileSync(join(runsDir, taskId, 'pr-readiness.md'), 'utf-8');
      assert(report.includes('PR Title Suggestion'), `Expected PR title section`);
      assert(report.includes('PR readiness test'), `Expected PR title text`);
    } finally {
      cleanup();
    }
  });

  test('pr-readiness report contains manual gh command as text only', () => {
    const { taskId, tasksFilePath, runsDir, cleanup } = createTempEnv({ withState: true, withApprovalReport: true });
    try {
      runCli(['real-repo-pr-readiness', taskId], {
        TASKS_FILE: tasksFilePath,
      ...
      const report = readFileSync(join(runsDir, taskId, 'pr-readiness.md'), 'utf-8');
      assert(report.includes('gh pr create'), `Expected gh command in report`);
      assert(report.includes('not executed by this tool'), `Expected disclaimer`);
    } finally {
      cleanup();
    }
  });

  test('pr-body contains task id', () => {
    const { taskId, tasksFilePath, runsDir, cleanup } = createTempEnv({ withState: true, withApprovalReport: true });
    try {
      runCli(['real-repo-pr-readiness', taskId], {
        TASKS_FILE: tasksFilePath,
      ...
      const body = readFileSync(join(runsDir, taskId, 'pr-body.md'), 'utf-8');
      assert(body.includes(taskId), `Expected task id in pr-body`);
    } finally {
      cleanup();
    }
  });

  test('pr-body contains goal', () => {
    const { taskId, tasksFilePath, runsDir, cleanup } = createTempEnv({ withState: true, withApprovalReport: true });
    try {
      runCli(['real-repo-pr-readiness', taskId], {
        TASKS_FILE: tasksFilePath,
      ...
      const body = readFileSync(join(runsDir, taskId, 'pr-body.md'), 'utf-8');
      assert(body.includes('Test goal'), `Expected goal in pr-body`);
    } finally {
      cleanup();
    }
  });

  test('pr-body contains pushed commit SHA', () => {
    const { taskId, tasksFilePath, runsDir, cleanup } = createTempEnv({ withState: true, withApprovalReport: true });
    try {
      runCli(['real-repo-pr-readiness', taskId], {
        TASKS_FILE: tasksFilePath,
      ...
      const state = JSON.parse(readFileSync(join(runsDir, taskId, 'state.json'), 'utf-8'));
      const body = readFileSync(join(runsDir, taskId, 'pr-body.md'), 'utf-8');
      assert(body.includes(state.commit_sha), `Expected commit SHA in pr-body`);
    } finally {
      cleanup();
    }
  });

  test('pr-body contains human review required', () => {
    const { taskId, tasksFilePath, runsDir, cleanup } = createTempEnv({ withState: true, withApprovalReport: true });
    try {
      runCli(['real-repo-pr-readiness', taskId], {
        TASKS_FILE: tasksFilePath,
      ...
      const body = readFileSync(join(runsDir, taskId, 'pr-body.md'), 'utf-8');
      assert(body.includes('human review'), `Expected human review in pr-body`);
    } finally {
      cleanup();
    }
  });
  ...
  test('report contains This tool did not call GitHub API', () => {
    const { taskId, tasksFilePath, runsDir, cleanup } = createTempEnv({ withState: true, withApprovalReport: true });
    try {
      runCli(['real-repo-pr-readiness', taskId], {
        TASKS_FILE: tasksFilePath,
      ...
      const report = readFileSync(join(runsDir, taskId, 'pr-readiness.md'), 'utf-8');
      assert(report.includes('This tool did not call GitHub API'), `Expected safety statement`);
    } finally {
      cleanup();
    }
  });

  test('report contains This tool did not run gh', () => {
    const { taskId, tasksFilePath, runsDir, cleanup } = createTempEnv({ withState: true, withApprovalReport: true });
    try {
      runCli(['real-repo-pr-readiness', taskId], {
        TASKS_FILE: tasksFilePath,
      ...
      const report = readFileSync(join(runsDir, taskId, 'pr-readiness.md'), 'utf-8');
      assert(report.includes('This tool did not run gh'), `Expected safety statement`);
    } finally {
      cleanup();
    }
  });
  ...
  test('command does not call GitHub API', () => {
    const { taskId, tasksFilePath, runsDir, cleanup } = createTempEnv({ withState: true, withApprovalReport: true });
    try {
      const result = runCli(['real-repo-pr-readiness', taskId], {
        TASKS_FILE: tasksFilePath,
      ...
      assert(result.stderr.includes('No GitHub API call was made'), `Expected no GitHub API message`);
    } finally {
      cleanup();
    }
  });
  ...
  test('command does not execute gh', () => {
    const { taskId, tasksFilePath, repoPath, runsDir, cleanup } = createTempEnv({ withState: true, withApprovalReport: true });
    try {
  ...
  test('command only writes pr-readiness.md and pr-body.md', () => {
    const { taskId, tasksFilePath, runsDir, cleanup } = createTempEnv({ withState: true, withApprovalReport: true });
    try {
      runCli(['real-repo-pr-readiness', taskId], {
        TASKS_FILE: tasksFilePath,
      ...
      assert(files.includes('pr-readiness.md'), `Expected pr-readiness.md`);
      assert(files.includes('pr-body.md'), `Expected pr-body.md`);
      ...
      assert.strictEqual(files.length, 4, `Expected exactly 4 files in run dir, got: ${files.join(', ')}`);
    } finally {
      cleanup();
    }
  });

  test('diff stat unavailable still writes reports with warning', () => {
    const { taskId, tasksFilePath, repoPath, runsDir, cleanup } = createTempEnv({ withState: true, withApprovalReport: true });
    try {
      ...
      assert.notStrictEqual(result.status, 0, `Expected failure because base branch deleted: ${result.stderr}`);
    } finally {
      cleanup();
    }
  });
  ...
  test('fake API key not printed', () => {
    ...
    try {
      ...
      assert.strictEqual(result.status, 0, `Expected success: ${result.stderr}`);
      assert(!result.stderr.includes('sk-fake-test-key-12345'), `Stderr must not contain API key`);
    } finally {
      cleanup();
    }
  });

  test('fake API key not written to reports', () => {
    const { taskId, tasksFilePath, runsDir, cleanup } = createTempEnv({ withState: true, withApprovalReport: true });
    try {
      runCli(['real-repo-pr-readiness', taskId], {
        TASKS_FILE: tasksFilePath,
      ...
      assert(!body.includes('sk-fake-test-key-12345'), `PR body must not contain API key`);
    } finally {
      cleanup();
    }
  });

  test('remote URL with credentials not written to reports', () => {
    const { taskId, tasksFilePath, repoPath, runsDir, cleanup } = createTempEnv({ withState: true, withApprovalReport: true });
    try {
      ...
      assert(!body.includes('user:pass'), `PR body must not contain credentials`);
    } finally {
      cleanup();
    }
  });
  ...
  test('existing readiness behavior unchanged', () => {
    ...
    try {
      const originPath = join(process.cwd(), 'tmp', `prr-r-origin-${Date.now()}`);
      spawnSync('git', ['init', '--bare', originPath], { encoding: 'utf-8', shell: false });
      spawnSync('git', ['remote', 'add', 'origin', originPath], { cwd: repoPath, encoding: 'utf-8', shell: false });
