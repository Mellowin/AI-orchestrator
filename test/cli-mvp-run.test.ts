...
describe('mvp-run product flow', () => {
  ...
  test('dependency failure blocks PR creation', async () => {
    const repo = createTempGitRepo();
    const runId = `dep-fail-no-pr-${Date.now()}`;
    const config: MvpRunConfig = {
      ...baseConfig(repo.path, runId),
      provider: 'kimi',
      allow_real_provider: true,
    ...
    const acceptedReview = (summary: string) =>
      JSON.stringify({
        decision: 'accepted',
        confidence: 'high',
        blocking_issues: [],
    ...
    // task_1 fails to parse; task_2 is skipped because its dependency failed.
    process.env.REAL_BLOCK_TASK_KIMI_FAKE_RESPONSES = JSON.stringify(['not-valid-json', fileUpdate('b.txt', 'b\n')]);
    process.env.REAL_BLOCK_TASK_REVIEWER_FAKE_RESPONSES = JSON.stringify([null, acceptedReview('Task 2 good')]);
    ...
    try {
      ...
      assert.ok(result.pr);
      assert.strictEqual(result.pr?.created, false);
      assert.ok(result.pr?.reason.includes('not attempted'), 'PR creation must not be attempted for a failed block');
