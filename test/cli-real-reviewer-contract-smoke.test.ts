...
describe('real-reviewer-contract-smoke CLI', () => {
  ...
  test('CLI source wires reviewer contract smoke branch without shell:true', () => {
    ...
    const nextBranchIndex = source.indexOf("if (!command || (!taskId && command !== 'real-repo-follow-up' && command !== 'real-block-follow-up' && command !== 'operator-e2e' && command !== 'diagnose-ci' && command !== 'doctor' && command !== 'autopilot-run' && command !== 'autopilot-plan' && command !== 'autopilot-one-click'))", branchIndex);
