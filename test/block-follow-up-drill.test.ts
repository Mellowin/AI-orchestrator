...
const DRILL_SCRIPT = join(PROJECT_ROOT, 'scripts', 'run-block-follow-up-drill.mjs');
...
function parseScenarioTable(output: string): Array<{
  ...
  for (const line of lines) {
    ...
    const [name, manual, files, repoOk, redact, passFail] = parts;
    rows.push({
      name,
      manual,
      files,
...
describe('block follow-up drill', () => {
  it('drill script exists', () => {
    const content = readFileSync(DRILL_SCRIPT, 'utf8');
    assert.ok(content.length > 0);
    assert.ok(content.includes('runScenarioA'));
    assert.ok(content.includes('real-block-follow-up'));
  });

  it('package.json has demo:block-follow-up-drill script', () => {
    const pkg = JSON.parse(readFileSync(join(PROJECT_ROOT, 'package.json'), 'utf8'));
    assert.ok(pkg.scripts?.['demo:block-follow-up-drill']);
    assert.ok(pkg.scripts['demo:block-follow-up-drill'].includes('run-block-follow-up-drill'));
  });
  ...
  it('exits non-zero when a scenario is forced to fail', () => {
    const result = runDrill({ BLOCK_FOLLOW_UP_DRILL_FORCE_FAIL: 'A', BLOCK_FOLLOW_UP_DRILL_KEEP_TEMP: '1' });
  ...
  it('output redacts token-like strings', () => {
    const result = runDrill({ BLOCK_FOLLOW_UP_DRILL_FORCE_FAIL: 'E', BLOCK_FOLLOW_UP_DRILL_KEEP_TEMP: '1' });
    const output = `${result.stdout}\n${result.stderr}`;
    assert.ok(!output.includes('secret-block-follow-up-key'), 'raw api_key value should be redacted from output');
    assert.ok(!output.includes('secret-block-follow-up-token'), 'raw token value should be redacted from output');
  ...
  it('follow-up task file redacts token-like strings', () => {
    ...
    const followUpFilePath = join(workspace, 'E', 'runs', 'task_2', 'follow-up-task_2-follow-up.yaml');
    ...
    assert.ok(!content.includes('secret-block-follow-up-key'), 'raw api_key should be redacted in follow-up file');
    assert.ok(!content.includes('secret-block-follow-up-token'), 'raw token should be redacted in follow-up file');
  ...
  it('cleans temp workspace by default', () => {
    ...
    const output = `${result.stdout}\n${result.stderr}`;
    assert.equal(result.status, 0, `drill should pass: ${output}`);
    const match = output.match(/Block follow-up drill workspace: (.+)/);
  ...
  it('preserves temp workspace when BLOCK_FOLLOW_UP_DRILL_KEEP_TEMP=1', () => {
    const result = runDrill({ BLOCK_FOLLOW_UP_DRILL_KEEP_TEMP: '1' });
    const output = `${result.stdout}\n${result.stderr}`;
    assert.equal(result.status, 0, `drill should pass: ${output}`);
