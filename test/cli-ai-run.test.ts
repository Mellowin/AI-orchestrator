...
describe('cli ai-run', () => {
  ...
  test('ai-run fails when preview guardrails fail', () => {
    const tmpTasks = createTempTasksFile({
      prefix: 'ai-run',
      taskId: TASK_ID,
      allowModify: ['package.json'],
      maxLinesChanged: 1,
    });
    cleanOutput(TASK_ID);
    try {
      const result = runAiRun(TASK_ID, [], {
        AI_PROVIDER: 'mock',
        MOCK_AI_RESPONSE: '{"mode":"file_update","files":[{"path":"package.json","content":"{\\"name\\":\\"shrunk\\"}\\n"}]}',
        TASKS_FILE: tmpTasks,
      });
      assert.notStrictEqual(result.status, 0, `Expected failure, got stderr: ${result.stderr}`);
      assert(
      ...
      assert(
        ...
        `Expected ai-generate ok, got stdout: ${result.stdout}`
      );
      assert(
        result.stdout.includes('[ai-run] Step 2/3: ai-validate'),
        `Expected step 2 log, got stdout: ${result.stdout}`
      ...
      assert(
        result.stdout.includes('[ai-run] ai-validate: ok'),
        `Expected ai-validate ok, got stdout: ${result.stdout}`
      );
      assert(
        result.stdout.includes('[ai-run] Step 3/3: ai-preview'),
        `Expected step 3 log, got stdout: ${result.stdout}`
      ...
      assert(
        !result.stdout.includes('[ai-run] ai-preview: ok'),
        `Should not show ai-preview ok after guardrail failure, got stdout: ${result.stdout}`
      );
      assert(
        !result.stdout.includes('[ai-run] Done.'),
        `Should not show Done after guardrail failure, got stdout: ${result.stdout}`
      );
      const combined = result.stdout + result.stderr;
      assert(
        combined.includes('Guardrails failed'),
        `Expected Guardrails failed, got: ${combined}`
      );
      assert(
        combined.includes('Proposed file line delta too large'),
        `Expected delta message, got: ${combined}`
      );
