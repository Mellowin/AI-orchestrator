...
describe('cli block-pr-draft', () => {
  let blockJsonPath: string;
  let blockId: string;
  let repoPath: string;
  let customOutputDir: string;

  function makeDefinition(): BlockDefinition {
    return {
      block_id: blockId,
      title: 'CLI PR Draft Test',
      repo_path: repoPath,
      base_branch: 'main',
  ...
  beforeEach(() => {
    ...
    customOutputDir = join(tmpdir(), `draft-${blockId}`);
  });
  ...
  test('missing block path shows safe error', () => {
    const result = runCli(['block-pr-draft']);
  ...
  test('missing block state fails safely', () => {
    ...
    const result = runCli(['block-pr-draft', blockJsonPath]);
    assert.strictEqual(result.status, 1, `Expected exit 1, got ${result.status}. stderr: ${result.stderr}`);
    const out = result.stdout + result.stderr;
  ...
  test('output includes output dir', () => {
    const def = makeDefinition();
    writeFileSync(blockJsonPath, JSON.stringify(def, null, 2));
    const state = makeCompletedState(def);
    saveBlockState(state);
    ...
    assert.ok(result.stdout.includes('Output dir:'), result.stdout);
  });

  test('output includes title/body/checklist paths', () => {
    const def = makeDefinition();
    writeFileSync(blockJsonPath, JSON.stringify(def, null, 2));
    const state = makeCompletedState(def);
    saveBlockState(state);
    const result = runCli(['block-pr-draft', blockJsonPath]);
    assert.strictEqual(result.status, 0, result.stderr);
    assert.ok(result.stdout.includes('Title:'), result.stdout);
    assert.ok(result.stdout.includes('Body:'), result.stdout);
    assert.ok(result.stdout.includes('Checklist:'), result.stdout);
    assert.ok(result.stdout.includes('pr-title.txt'), result.stdout);
    assert.ok(result.stdout.includes('pr-body.md'), result.stdout);
    assert.ok(result.stdout.includes('manual-pr-checklist.md'), result.stdout);
  });
  ...
  test('custom output env works', () => {
    ...
    saveBlockState(state);
    const result = runCli(['block-pr-draft', blockJsonPath], {
      BLOCK_PR_DRAFT_OUTPUT_DIR: customOutputDir,
    });
    assert.strictEqual(result.status, 0, `Expected exit 0, got ${result.status}. stderr: ${result.stderr}`);
    assert.ok(result.stdout.includes(customOutputDir) || result.stdout.includes('Output dir:'), result.stdout);
    assert.ok(existsSync(join(customOutputDir, 'pr-title.txt')), 'title missing');
    assert.ok(existsSync(join(customOutputDir, 'pr-body.md')), 'body missing');
    assert.ok(existsSync(join(customOutputDir, 'manual-pr-checklist.md')), 'checklist missing');
  });
