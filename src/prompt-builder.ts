...
function buildDependencyEvidenceSection(context: ContextPackage): string {
  if (!context.dependency_evidence || context.dependency_evidence.items.length === 0) {
    return '';
  }
  const evidence = context.dependency_evidence;
  const items = evidence.items
    .map(
      (item) =>
        `### ${item.path} (from ${item.task_id}, ${item.task_status})\n` +
        `sha256: ${item.content_sha256} | bytes: ${item.bytes} | lines: ${item.lines}${item.truncated ? ' [truncated]' : ''}\n\n` +
        wrapInCodeFence(item.content)
    )
    .join('\n\n');

  return (
    `\n\n# Dependency Evidence (read-only)\n\n` +
    `The following files are from previously accepted ancestor tasks. They are provided as context only. ` +
    `You may NOT modify them. You may only write to files listed in the Allowed Files section above.\n\n` +
...
export function buildKimiPrompt(context: ContextPackage): string {
  ...
  const lineLimitSection =
    context.max_lines_changed !== undefined
      ? `\n\n# Line change budget\n\n` +
        `Advisory budget: prefer to keep any single file change under ${context.max_lines_changed} lines. ` +
