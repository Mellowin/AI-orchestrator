...
- Перед пропуском уже завершённой задачи на `resume` проверяй, что её коммит действительно присутствует в истории текущей рабочей ветки (`src/block/block-state-consistency.ts`). Если коммит отсутствует или не является предком `HEAD`, считай состояние устаревшим и не используй его.
- `operator-e2e` дополнительно сверяет `artifactConsistency`: число принятых задач и их коммитов должно совпадать с ожидаемым, иначе PR не создаётся.
...
- Детерминированный resume (Stage 18.26f): после первого успешного планирования мультизадачной миссии оригинальный plan result сохраняется НЕИМЕНЯЕМО в `missions/<run_id>/resume-plan-snapshot.json` (`src/autopilot-one-click/resume-plan-snapshot.ts`). Resume с тем же `--run-id` загружает точный исходный план и делает НОЛЬ вызовов planner (провайдер недетерминирован, перегенерация меняет plan hash и ломает resume). Identity/integrity проверки (run_id, repo, base branch, goal, mode, hash снапшота, `state.plan_hash` против снапшота) выполняются ДО любой перезаписи артефактов; при провале — fail-closed без provider-вызовов, evidence добавляется append-only в `resume-attempts.jsonl`. Миссия с execution state, но без снапшота (legacy, до 18.26f), fail-closed как `LEGACY_RESUME_PLAN_UNAVAILABLE` — исходный план НИКОГДА не реконструируется через AI. Проверка `base_sha` в multitask runner сохраняется и независима от планировщика.
