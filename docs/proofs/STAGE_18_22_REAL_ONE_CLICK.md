...
npx tsx src/cli.ts autopilot-one-click \
  "Create a docs/proofs/STAGE_18_22_REAL_ONE_CLICK.md marker file for Stage 18.22" \
  --preset real-pr \
  --mode github \
  --repo-slug Mellowin/AI-orchestrator \
  --repo-path tmp/stage-18-22-repo \
  --base-branch stage-18-21-raw-goal-one-click-ux \
  --output-dir reports/autopilot-one-click \
  --yes \
  --allowed-files docs/proofs/STAGE_18_22_REAL_ONE_CLICK.md
```
...
- **CI status at time of writing:** `in_progress` / queued
...
- The GitHub Actions workflow run remained queued/in-progress beyond the observation window. No CI conclusion was observed.
...
`REAL_RAW_GOAL_ONE_CLICK_READY_WITH_CAVEATS` — the real raw-goal one-click path created a mission, plan, commit, branch, push, and PR. CI observation was attempted but did not complete within the available window.
